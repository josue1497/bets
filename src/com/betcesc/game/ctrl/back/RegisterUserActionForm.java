...
public class RegisterUserActionForm extends ActionForm implements UsuarioIF {

	private static final long serialVersionUID = 4777760968541305564L;

	private static final Log log = LogFactory.getLog(RegisterUserActionForm.class);
	...
	public ActionErrors validate(ActionMapping arg0, HttpServletRequest request) {
		...
		if (!Validator.isInteger(this.getCedula()) || Integer.parseInt(this.getCedula()) < 3000000 || this.getCedula().length() <= 3) {
			errores.add("cedula", new ActionError("error.field.validate", "cedula"));
		}
		...
		if (!Validator.isString(this.getApellido()) || this.getApellido().length() < 3) {
			errores.add("apellido", new ActionError("error.field.validate", "apellido"));
		}

		if (Validator.isEmpty(this.getUsuario())) {
			errores.add("usuario", new ActionError("error.field.required", "usuario"));
		}

		if (this.getUsuario().length() < 3) {
			errores.add("usuario", new ActionError("error.field.validate", "usuario"));
		}

		if (Validator.isEmpty(this.getClave())) {
		...
		if (!Validator.isEmpty(this.getLogrosAltaBaja()) && !Validator.isInteger(this.getLogrosAltaBaja())) {
			errores.add("logrosAltaBaja", new ActionError("error.field.validate", "logros A/B"));
		}

		if (!Validator.isEmpty(this.getLogrosCalc()) && !Validator.isInteger(this.getLogrosCalc())) {
			errores.add("logrosCalc", new ActionError("error.field.validate", "logros Calculadora"));
		}

		if (!Validator.isEmpty(this.getMonto()) && !Validator.isInteger(this.getMonto())) {
			errores.add("monto", new ActionError("error.field.validate", "monto"));
		}

		if (!Validator.isEmpty(this.getTelefono()) && !Validator.isInteger(this.getTelefono())) {
			errores.add("telefono", new ActionError("error.field.validate", "telefono"));
		}

		if (!Validator.isEmpty(this.getCelular()) && !Validator.isInteger(this.getCelular())) {
			errores.add("celular", new ActionError("error.field.validate", "celular"));
		}

		if (Validator.isEmpty(this.getCorreo())) {
			errores.add("correo", new ActionError("error.field.required", "correo"));
		}

		if (!Validator.isEmpty(this.getNumeroCuenta()) && !Validator.isInteger(this.getNumeroCuenta())) {
			errores.add("numeroCuenta", new ActionError("error.field.validate", "Nro de Cuenta"));
		}

		if (request.getParameter("eliminar") != null && request.getParameter("eliminar").equals("true")) {
			if (this.getIdUsuario() != null && this.getIdUsuario().trim().equals("1")) {
				errores.add("idUsuario", new ActionError("error.sistema.administrador"));
			}
		}
