...
public class RegisterGameMultiAction extends Action {

	private static final Log log = LogFactory.getLog(RegisterGameMultiAction.class);
		...
		try {
			...
			if(request.getParameter("idJuego")!=null && !request.getParameter("idJuego").trim().equals("") && !request.getParameter("idJuego").trim().equals("0") ) {
				...
				*/
				...
				// quitamos las opciones anteriores de la session;
				request.getSession().removeAttribute("forma");
				
				/* vamos a cargar algunas tablas de uso general */
				request.setAttribute("listaDeporte",oJuegoFacade.listaDeporteMultipleFacade(oJuegoFacade.listaDeporteFacade(), true));
			...
			if(request.getAttribute("listaDeporte")==null) {
				request.setAttribute("listaDeporte",oJuegoFacade.listaDeporteMultipleFacade(oJuegoFacade.listaDeporteFacade(false),true));
			}
