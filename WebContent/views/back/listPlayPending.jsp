...
function send(orden) {
	document.forms[0].action="listPlayPending.do";
...
<form name="forma" action="listPlayPending.do">
...
<table align="center" width="98%" height="100%" >
		...
		<td colspan="<%=viewUser?8:7%>">
			<table align="center" width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td width="20%" class="tituloTablaSup" align="center" >LISTA DE TICKETS GANADORES PENDIENTES POR PAGAR</td>
					<td width="40%" align="right">
						<table height="100%" border="0" cellpadding="0" cellspacing="0" class="none">
						  <tr>
							<td class="detalleTablaRight">Fecha :&nbsp;</td>
							<td>
				  			<input type="text" name="fechaJugada" size="10" maxlength="10" value="<%=session.getAttribute("fechaJugadaPendiente")%>" readOnly />
							</td>
							<td>	
							        <a href="javascript:document.forms[0].fechaJugada.value='';show_calendar('forms[0].fechaJugada', 'forms[0].fechaJugada', 'forms[0].fechaJugada');" 
	...
	<tr>
		...
		<%}%>
		<td class="tituloTabla tablaConBorde"><bean:message key="jugada.montoApostado"/></td>
		<td class="tituloTabla tablaConBorde"><bean:message key="jugada.montoPremio"/></td>
		<td class="tituloTabla tablaConBorde" ><bean:message key="jugada.montoPagado"/></td>
	...
	<tr onmouseover="this.className='lineaResaltada'" onmouseout="this.className=''" onclick="abrir('<%=lista.getString("id_jugada")%>')">
		...
		<td class="detalleTablaLeft <%=Constants.getColorStatusJugada(lista.getString("id_status_jugada"))%>">&nbsp;<%=lista.getString("desc_jugada")%><%=Integer.parseInt(lista.getString("bono"))>0?" (BONO)":""%></td>
		<%if(viewUser) {%>
