...
<table align="center" width="98%" cellspacing="0" cellpadding="0"
	...
	<tr>
		...
		<td rowspan="2" align="center"><img
			src="<%=Constants.getDominio(request).getPathImages()%><%=request.getAttribute("titulo").toString().toLowerCase().replaceAll(" ", "_")%>.gif">
		</td>
		<%
			}
		%>
		<td class="tituloTablaBig">
			...
			%> <img
			src="<%=Constants.getDominio(request).getPathImages()%>logoLogro.jpg" />
			<%
				}
			%>
		</td>
	...
	<tr>
		<td class="tituloTablaBig"><%=request.getAttribute("titulo")%></td>
	</tr>
</table>
<table align="center" width="98%" cellspacing="0" cellpadding="0"
	...
	<logic:present name="lista" scope="request">
			...
			CachedRowSet lista = (CachedRowSet) request.getAttribute("lista");
				...
				while (lista.next()) {
					...
					if (idJuego == null || !idJuego.equals(lista.getString("id_juego"))) {
						...
						if (titulo == null
								|| !titulo.equals(lista.getString("dia").concat(lista.getString("desc_liga")))) {
							subtitulo = true;
		%>
		<tr>
			<td class="tit" colspan="<%=isActiveSRL ? 23 : 20%>">&nbsp;<%=lista.getString("desc_liga")%>
				(<%=lista.getString("iniciales")%>) - <%=lista.getString("dia")%>
			</td>
		</tr>
		<%
			} else {
		...
		<%
			if (subtitulo && uo.equals(Constants.ALTA)) {
						isPitcher = lista.getString("id_deporte").equals(Constants.ID_EQUIPO_BEISBOL)
								|| lista.getString("id_deporte").equals(Constants.ID_EQUIPO_BEISBOL_5);
		%>
		<tr>
			...
			<td class="tit" colspan="2">Anota 1ro</td>
		</tr>
		<tr>
			...
			<td class="tit" colspan="2">Carreras</td>
		</tr>
		<tr>
			<td class="tit">Ref</td>
		...
		%>
		<tr bgcolor="<%=color%>" class="det">
			<!--class="detalleTablaCenterBW"> -->
			<%
				if (uo.equals(Constants.ALTA)) {
			%>
			<td width="4%"
				<%=uo.equals(Constants.BAJA)
										? (lista.getString("status_real").equals("Abierto/Taq.")
												? "class='yellow'"
												: (lista.getString("status_real").startsWith("Totalizado")
														? "class='blue'"
														: (lista.getString("status_real").startsWith("Borrador")
																? "class='green'"
																: (lista.getString("status_real")
																		.startsWith("Abierto/Bloq.")
																				? "class='agua'"
																				: "class='red'"))))
										: ""%>
			...
			%>
			<td
				class="border<%=uo%> <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"
				align="left"><%=lista.getString("desc_equipo")%><%=isPitcher ? "<br/>" : ""%><i><%=Constants.isNull(lista.getString("nombre_lanzador"), "", "")%></i>
			</td>
			<td
				class="border<%=uo%> <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"><b><%=lista.getString("referencia")%></b></td>
			<td
				class="border<%=uo%> printLogro <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"><%=Constants.signo(lista.getString("money_line"))%></td>
			<td id="A_<%=uo + idJuego%>"
				class="border<%=uo%> cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">&nbsp;</td>
			<td id="B_<%=uo + idJuego%>"
				class="border<%=uo%> colorLogro5to  cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">&nbsp;</td>
			<td
				class="border<%=uo%> <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"><b>&nbsp;
					<%
						entrar = ((!Constants.tipoFraccion(lista.getString("spread")).equals("")
										|| !Constants.signo(lista.getString("spread_logro")).equals(""))
										&& !Constants.signo(lista.getString("spread_logro")).equals("0"));
								if (entrar) {
 ...
 %>
			...
			%>
			<td
				class="border<%=uo%> printLogro <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">
				&nbsp; <%
 	if (entrar) {
 %> <%=Constants.tipoFraccion(lista.getString("spread")).equals("")
									? "0"
									: Constants.tipoFraccion(lista.getString("spread"))%>
				<%
					}
				%>
			</td>
			...
			<td id="D_<%=uo + idJuego%>"
				class="border<%=uo%> colorLogro5to cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">&nbsp;</td>
			<td id="E_<%=uo + idJuego%>"
				class="border<%=uo%> printLogro cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">&nbsp;</td>
 	...
 	entrar = ((!Constants.tipoFraccion(lista.getString("super_spread")).equals("")
 							|| !Constants.signo(lista.getString("super_spread_logro")).equals(""))
 							&& !Constants.signo(lista.getString("super_spread_logro")).equals("0"));
 					if (entrar) {
 ...
 %>
			...
			<td class="border<%=uo%> printLogro">
				<%
					if (entrar) {
				%> <%=Constants.tipoFraccion(lista.getString("super_spread")).equals("")
										? "0"
										: Constants.tipoFraccion(lista.getString("super_spread"))%>
				<%
					} else {
										out.print("&nbsp;");
									}
 ...
 %> <%=Constants.signo(lista.getString("super_spread_logro"))%>
			...
			<td colspan="2" align="center" style="border-bottom: 0px;"
				valign="bottom">&nbsp; <%
 	entrar = (!Constants.tipoFraccion(lista.getString("total"), false).equals("")
 							&& !Constants.signo(lista.getString("total_logro")).equals("0"));
 					if (entrar) {
 %> <b><%=Constants.tipoFraccion(lista.getString("total"), false)%></b>
				</br>
			<b><%=lista.getString("referencia_ab")%>:</b><span
				class="printLogro <%=isEmpate
										? "borderEmpate"
										: (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"><%=Constants.signo(lista.getString("total_logro"))%></span>
				<%
					}
				%>
			</td>
			...
			<td colspan="2" style="border-top: 0px;" valign="top"
				class="<%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">
				&nbsp; <%
 	entrar = (!Constants.tipoFraccion(lista.getString("total"), false).equals("")
 							&& !Constants.signo(lista.getString("total_logro")).equals("0"));
 					if (entrar) {
 %> <b><%=lista.getString("referencia_ab")%>:</b><span
				class="printLogro <%=isEmpate
										? "borderEmpate"
										: (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>"><%=Constants.signo(lista.getString("total_logro"))%></span>
				<%
					}
				%>
			</td>
			...
			<%
				if (uo.equals(Constants.ALTA)) {
			%>
			<td id="F_<%=uo + idJuego%>" colspan="2"
				class="cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>"
				align="center" style="border-bottom: 0px;" valign="bottom">&nbsp;</td>
			<%
				} else if (uo.equals(Constants.BAJA)) {
			%>
			<td id="G_<%=uo + idJuego%>" colspan="2"
				class="cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>"
				style="border-top: 0px;" valign="top">&nbsp;</td>
			<%
				} else if (!uo.equals(Constants.BAJA)) {
			%>
			<td>&nbsp;</td>
			...
			%>
			<td id="H_<%=uo + idJuego%>"
				style="<%=uo.equals(Constants.BAJA)
								? "border-top:0px;vertical-align:top;"
								: "border-bottom:0px;vertical-align:bottom;"%>"
				class=" <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateGray" : "")%>">&nbsp;</td>
			<td id="I_<%=uo + idJuego%>"
				style="<%=uo.equals(Constants.BAJA)
								? "border-top:0px;vertical-align:top;"
								: "border-bottom:0px;vertical-align:bottom;"%>"
			...
			%>
			<td
				class="<%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			<td
				class="<%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			<td
				class="colorLogro5to  cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			<td
				class="colorLogro5to  cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			<td
				class="colorLogro5to  cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			...
			%>
			<td
				class="colorLogro5to  cinco <%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>"
				colspan="2">&nbsp;</td>
			<td
				class="<%=isEmpate ? "borderEmpate" : (uo.equals(Constants.BAJA) ? "borderEmpateVacio" : "")%>">&nbsp;</td>
			<td
	...
	<logic:present name="lista5to" scope="request">
		<table style="display: none">
			...
			<%
							...
							titulo = String.valueOf(lista.getString("dia"))
									.concat(String.valueOf(lista.getString("desc_liga")));
						}
 ...
 %> <%=Constants.signo(lista.getString("spread_logro"))%>
				...
				%>
				<td id="<%="5F_".concat(idColumna)%>" colspan="2" align="center"
					style="border-bottom: 0px;" valign="bottom">&nbsp; <%
 	entrar = (!Constants.tipoFraccion(lista.getString("total"), false).equals("")
 							&& !Constants.signo(lista.getString("total_logro")).equals("0"));
 					if (entrar) {
 %> <b><%=Constants.tipoFraccion(lista.getString("total"), false)%></b>
				...
				<b><%=lista.getString("referencia_ab")%>:</b><span
					class="colorLogro5to"><%=Constants.signo(lista.getString("total_logro"))%></span>
					<%
						}
					%>
				</td>
				...
				%>
				<td id="<%="5G_".concat(idColumna)%>" colspan="2"
					style="border-top: 0px;" valign="top">&nbsp; <%
 	entrar = (!Constants.tipoFraccion(lista.getString("total"), false).equals("")
 							&& !Constants.signo(lista.getString("total_logro")).equals("0"));
 					if (entrar) {
 %> <b><%=lista.getString("referencia_ab")%>:</b><span
...
<center>
	<span class="final"> <%=request.getAttribute("PIE_PAGINA_LOGRO")%>
	</span>
</center>
