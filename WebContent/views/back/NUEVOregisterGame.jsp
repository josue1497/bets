...
alert(1);
		...
		var textoCrudo = text.value;
		
		textoCrudo =  replaceAll("Línea", " ", textoCrudo);
		textoCrudo =  replaceAll("LÍNEA", " ", textoCrudo);
		textoCrudo =  replaceAll("Props", "", textoCrudo);
		textoCrudo =  replaceAll("Total", "", textoCrudo);
		textoCrudo =  replaceAll("Resultado", "", textoCrudo);
		textoCrudo =  replaceAll("Equipos", "", textoCrudo);
		
		
		
		var juegos = textoCrudo.split('Estadísticas');
		text.value  ="";
		var horaProximo ="";
		var fechaJuegos ="";
		...
		for ( i=0; i<juegos.length; i++ )
			...
			if (i==0){ //Es Encabezado
				horaProximo= juegos[i].substring(juegos[i].lastIndexOf(":") - 2, juegos[i].length ).replace(/(\r\n|\n|\r)/gm," ");
				
				var fecha= juegos[i].split(",");
				fechaJuegos=trim(fecha[1]);
				
				mesJuego =fechaJuegos.substring(0, 3).toLowerCase(); 
				
				inxEspacio = fechaJuegos.indexOf(" ");
				diaJuego =fechaJuegos.substring(inxEspacio , inxEspacio+5); 
				diaJuego = diaJuego.replace(/[^0-9\.]+/g, '');
				
				
				
				for(j=0; j<mes0.length; j++) {
					if (mes0[j]==mesJuego){
						mesJuego =mesesUS[j];
						break;
					} 
				}
				fechaJuegos = mesJuego + " " +diaJuego;  
				
				continue;				
			}
			
			
			juegos[i] = fechaJuegos + " " +horaProximo + " " + juegos[i].replace(/(\r\n|\n|\r)/gm," ");
			console.log(juegos[i]);
			indIndicaInicioHora=juegos[i].lastIndexOf(":") - 2;
			inicioHora= juegos[i].substring(indIndicaInicioHora);
			//console.log(inicioHora);
			horaProximo= juegos[i].substring(indIndicaInicioHora, juegos[i].length ).replace(/(\r\n|\n|\r)/gm,"");
			
			if (juegos[i].lastIndexOf(":")>12){
			
				juegos[i]= juegos[i].substring(0, indIndicaInicioHora );
				console.log("1 " + juegos[i]);
				juegos[i]=juegos[i].trim();
				console.log("2 " + juegos[i]);
				juegos[i]= juegos[i].substring(0, juegos[i].lastIndexOf(" "));
				console.log("3" + juegos[i]);
				juegos[i]= juegos[i].substring(0, juegos[i].lastIndexOf(" "));
				console.log("4 " + juegos[i]);
				
			}
			
			//juegos[i]=juegos[i].replace(/   /g, "  ");
			//juegos[i]=juegos[i].replace(/  /g, "  ");
			//juegos[i]=juegos[i].trim();
			
			//juegos[i] = juegos[i].substring(0,juegos[i].lastIndexOf(" ")-2);
			//console.log (juegos[i]);
			juegos[i] =  replaceAll("  ", " ", juegos[i]);
			juegos[i] =  replaceAll("  ", " ", juegos[i]);
			juegos[i] =  replaceAll("  ", " ", juegos[i]);
			juegos[i] =  replaceAll("  ", " ", juegos[i]);
			text.value  = text.value  + juegos[i] +"\n"  ;  
  	...
  	<td align="center" width="100%" valign="top">
		...
		<table width="100%" height="100%" border="0" cellpadding="5" cellspacing="5" class="tablaCaption">
			...
			<tr id="rowDraw">
			  	...
			  	<td class="detalleTablaCenter" >
			  		<html:select value="0" styleId="idLanzador2" property="_IdLanzador" style="width:95%" style="display:none">
			  			<html:option value="0">Seleccione un equipo...</html:option>
